...
TEST_CASE("functions#step_function", "[functions]") {
  ...
  X1 << 1.0, 0.5,
      2.0, 1.5;
  CHECK(functions::step_function(X1) == Eigen::MatrixXd::Ones(2, 2));

  Eigen::MatrixXd X2(2, 2);
  X2 << -1.0, -0.5,
      -2.0, -1.5;
  CHECK(functions::step_function(X2) == Eigen::MatrixXd::Zero(2, 2));

  Eigen::MatrixXd X3(2, 2);
  X3 << 1.0, -0.5,
      -2.0, 1.5;
  CHECK(functions::step_function(X3) == Eigen::MatrixXd::Identity(2, 2));
}
...
TEST_CASE("functions#sigmoid", "[functions]") {
  ...
  E << 0.731059, 0.622459,
      0.880797, 0.817574;
  CHECK(functions::sigmoid(A).isApprox(E, 1e-4));
}
